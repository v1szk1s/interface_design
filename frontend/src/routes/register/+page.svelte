...
<div class="container lg:w-1/2 mx-auto">
    <form method="POST" action="?/register" use:enhance>
        ...
        <div class="mb-3">
            <Label for="password2-input" class="block mb-2">Password again</Label>
            <Input id="password2-input" name="passwordConfirm" type="password" placeholder="..." color="{(form?.errors && form.errors.includes("passwordConfirm")) ? "red":""}"/>
            <Helper class='mt-2 {(form?.errors && form.errors.includes("passwordConfirm")) ? "visible":"invisible"}' color='red'>
                <span class="font-medium">Email must be filled</span>
            </Helper>
            {#if form?.errors.includes("notsame")}
